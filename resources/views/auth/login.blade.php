    ...
    <div class="mx-auto max-w-sm py-12">
        <div class="rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl">
            <div class="mb-6 text-center">
                ...
                <span class="kicker block text-[10px] font-bold uppercase text-emerald-400 mb-1">{{ __('auth.kicker') }}</span>
                <h1 class="text-xl font-black text-white">{{ __('auth.heading') }}</h1>
            </div>
            ...
            <form method="POST" action="{{ route('login.attempt') }}" class="space-y-4">
                @csrf
                <div>
                    <label for="email" class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('auth.email') }}</label>
                ...
                <div>
                    <label for="password" class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('auth.password') }}</label>
